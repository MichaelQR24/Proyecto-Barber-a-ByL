...
function normalizeTelefono(telefono) {
  ...
  // Si tiene menos de 9 dígitos después de limpiar, devolver tal cual
...
const authController = {
  ...
  register: (req, res) => {
    let { nombre, email, password, telefono, rol = 'cliente' } = req.body;
    
    // Normalizar el teléfono si se proporciona
    if (telefono) {
  ...
  updateProfile: (req, res) => {
    ...
    // Normalizar el teléfono si se está actualizando
    if (telefono) {
      telefono = normalizeTelefono(telefono);
    }
