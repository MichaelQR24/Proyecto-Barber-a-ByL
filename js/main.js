...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function actualizarUIPorAutenticacion() {
    ...
    // Definir variables para el menú de usuario
    ...
    if (token) {
      ...
      try {
        ...
        if (userRol === 'admin') {
          ...
          // Usuario cliente - Activar menú desplegable de usuario
          // Ocultar el botón de login original y mostrar el menú de usuario
          loginBtn.style.display = 'none';
          userMenu.style.display = 'block';
          
          // Obtener nombre del usuario del token o pedirlo a través de la API
          const userNameDisplay = document.getElementById('userNameDisplay');
          if (userNameDisplay) {
            // Usar el nombre del payload del token si está disponible
            if (tokenPayload.nombre) {
              const nombreParts = tokenPayload.nombre.split(' ');
              userNameDisplay.textContent = nombreParts[0];
            } else {
              // Si no está en el token, obtenerlo de la API
              apiClient.getProfile()
                .then(response => {
                  const nombreParts = response.user.nombre.split(' ');
                  userNameDisplay.textContent = nombreParts[0];
                })
                .catch(error => {
          ...
          // Configurar evento para el botón del menú de usuario
          if (userMenuBtn) {
        ...
        // Mantener el botón de login visible para manejar cierre de sesión
        if (btnText) {
          btnText.textContent = 'Cerrar Sesión';
        }
        loginBtn.href = '#';
  ...
  async function cargarServiciosEnFormulario() {
    try {
      ...
      servicios.forEach(servicio => {
        ...
        option.textContent = `${servicio.nombre} - S/ ${servicio.precio}`;
  ...
  function formatTelefono(telefono) {
    ...
    // Asegurar que tiene 9 dígitos
    if (cleanPhone.length < 9) {
      return telefono; // Devolver tal cual si no tiene suficientes dígitos
    }
    ...
    return `${phone9Digits.slice(0, 3)} ${phone9Digits.slice(3, 6)} ${phone9Digits.slice(6, 9)}`;
  }
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      // Validar que el teléfono tenga máximo 9 dígitos
      const phoneDigits = vals.phone.replace(/\D/g, ''); // Eliminar todos los caracteres no numéricos
      if (phoneDigits.length > 9) {
        alert("El teléfono debe tener máximo 9 dígitos.");
        return;
      }
      ...
      try {
        ...
        try {
          ...
          // Normalizar el número de teléfono antes de actualizar
          const telefonoNormalizado = vals.phone.replace(/\D/g, '').slice(-9);
          
          // Si tenemos un teléfono a actualizar, intentar actualizarlo
          if (telefonoNormalizado && telefonoNormalizado.length === 9) {
            try {
              // Intentar actualizar directamente el teléfono
              await apiClient.updateProfile({ telefono: telefonoNormalizado });
            } catch (updateError) {
              console.error('Error al actualizar el teléfono:', updateError);
            }
          }
        } catch (profileError) {
        ...
        servicios.forEach(servicio => {
          ...
          serviceCard.innerHTML = `
            ...
            <div class="svc-body">
              ...
              <span class="price">S/ ${servicio.precio.toLocaleString('es-PE')}</span>
            </div>
          `;
  ...
  // Funcionalidad para el menú desplegable de usuario
  const userMenu = document.getElementById('userMenu');
  const userMenuBtn = document.getElementById('userMenuBtn');
  const dropdownMenu = document.getElementById('dropdownMenu');
  const logoutLink = document.getElementById('logoutLink');
  ...
  function toggleDropdown() {
    ...
    const arrowIcon = userMenuBtn.querySelector('.arrow-down');
    if (arrowIcon) {
  ...
  // Evento click para mostrar/ocultar el menú desplegable
  if (userMenuBtn) {
    userMenuBtn.onclick = (e) => {
      e.preventDefault();
      toggleDropdown();
